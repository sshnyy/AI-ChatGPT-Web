...
import com.insert.project.WebSocketChatHandler;
...
import com.insert.project.domain.repository.BoardFileRepository;
...
public class BoardService {
    ...
    public void save(BoardDTO boardDTO, Member member) throws IOException {
        // 파일 첨부 여부에 따라 로직 분리
...
//            String storedFileName = originalFilename; // 3.
            String savePath = "/home/suyeon/code/capstone3/DL/image/spring/" + storedFileName; // 4. C:/springboot_img/9802398403948_내사진.jpg
//            String savePath = "/Users/사용자이름/springboot_img/" + storedFileName; // C:/springboot_img/9802398403948_내사진.jpg
            ...
            // 이미지 파일 아이디를 파이썬 서버로 전송
            try (Socket socket = new Socket("203.232.193.173", 5252);
                 PrintWriter out = new PrintWriter(socket.getOutputStream(), true)) {

                // 이미지 파일 아이디를 전송
                out.println("IMG " + savedId);
            } catch (IOException e) {
                // 소켓 통신 중 오류 발생 시 처리
                e.printStackTrace();
                // 오류 처리 로직 추가
    ...
    public void update(BoardDTO boardDTO,Long boardId) {
        BoardEntity findBoard = boardRepository.findById(boardId).orElseThrow(() -> new IllegalArgumentException());

        findBoard.setBoardTitle(boardDTO.getBoardTitle());
        findBoard.setBoardContents(boardDTO.getBoardContents());
    ...
    public Page<BoardDTO> paging(Pageable pageable) {
        ...
        // 'chat_log' 제목을 가진 게시글을 제외하고 결과 조회
        Page<BoardEntity> boardEntities = boardRepository.findAllExcludeChatLog("chat_log",
